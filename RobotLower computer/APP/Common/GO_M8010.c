...
static void GOM8010PackControlFrame(GOM8010MotorCmd_TypeDef *motor_s)
{
    ...
    float torque = motor_s->torque;
    float speed = motor_s->speed;
    float position = motor_s->position;
    float kp = motor_s->kp;
    float kd = motor_s->kd;
    int16_t torque_raw;
    int16_t speed_raw;
    ...
    if (id > 15U)      /* id占4bit */
        id = 15U;
    if (mode > 2U)     /* 0=disable,1=FOC电流环,2=位置/速度环 */
        mode = 2U;
    if (timeout > 1U)
        timeout = 1U;
    SATURATE(kp, 0.0f, 25.599f);              /* kp/kd满量程25.6,对应16bit定点 */
    SATURATE(kd, 0.0f, 25.599f);
    SATURATE(torque, -4.0f, 4.0f);            /* 电机额定最大转矩±4N*m */
    SATURATE(speed, -804.0f, 804.0f);         /* 最大转速±804rad/s,由16bit定点满量程反推 */
    SATURATE(position, -411774.0f, 411774.0f); /* 位置范围,由32bit定点满量程反推 */
...
static void GOM8010ParseFeedbackFrame(GOM8010MotorFeedback_TypeDef *motor_r)
{
    ...
    motor_r->torque = (float)torque_raw / 256.0f;
    motor_r->speed = (float)speed_raw / 256.0f * PI2;
    motor_r->position = (float)position_raw / 32768.0f * PI2;
    motor_r->valid = 1U;
}
